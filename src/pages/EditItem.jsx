...
export default function EditItem() {
    ...
    const load = async () => {
      ...
      try {
        ...
        setForm({
          ...
          conversion_factor: toFormValue(item?.conversion_factor),
          minimum_threshold: toFormValue(item?.minimum_threshold ?? 1),
          reorder_quantity: toFormValue(item?.reorder_quantity),
          max_stock_level: toFormValue(item?.max_stock_level),
        ...
        const chemical = Array.isArray(item?.item_chemical_details)
          ? item.item_chemical_details[0]
          : item?.item_chemical_details

        const equipment = Array.isArray(item?.item_equipment_details)
          ? item.item_equipment_details[0]
          : item?.item_equipment_details

        const reference = Array.isArray(item?.item_reference_details)
        ...
        setExt({
          ...
          molecular_weight: toFormValue(chemical?.molecular_weight),
          msds_url: chemical?.msds_url ?? "",
          pubchem_id: toFormValue(chemical?.pubchem_id),
          ghp_classification: chemical?.ghp_classification ?? "",

          model_number: equipment?.model_number ?? "",
          serial_number: equipment?.serial_number ?? "",
          maintenance_interval_days: toFormValue(equipment?.maintenance_interval_days),
          last_maintenance_date: normalizeDate(equipment?.last_maintenance_date),
          warranty_expiry: normalizeDate(equipment?.warranty_expiry),

          certification_number: reference?.certification_number ?? "",
          certification_expiry: normalizeDate(reference?.certification_expiry),
  ...
  const submit = async () => {
    ...
    try {
      await updateItem(id, buildPayload())
      navigate(`/items/${id}`)
    } catch (err) {
  ...
  if (error && !saving && !form.name && !form.sku) {
    return (
      <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg p-5">
        ...
        <div className="mt-4">
          <button
            ...
            className="px-4 py-2 text-sm rounded-lg border border-red-300 hover:bg-red-100"
          >
            Back to Items
          </button>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <Field
              ...
              hint="SKU is shown for reference and should not be changed casually."
            >
              <Input
                value={form.sku}
                onChange={set("sku")}
            ...
            <Field
              ...
              hint={selectedCategory?.is_hazardous ? "Selected category is marked hazardous" : null}
            >
              <Select
